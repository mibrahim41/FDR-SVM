...
k_vec = [0.1,0.25,0.5,0.75,1];
[accs_iter,accs_param,acc_cent] = gen_final_result(N_exp,beta_vec,k_vec);
str_ids = ["nom","gro","cla","two","wro"];
titles = ["Nominal","Client Imabalnce","Class Imbalance","Client + Class Imbalance","Noisy Labels"];

[accs_iter_bds,accs_param_bds,acc_central_bds] = gen_bounds(N_exp,beta_vec,k_vec);
...
for i = 1:length(str_ids)
    ...
    err_temp_central = ones([1,length(beta_vec)])*std(acc_central_bds(i,:));

    acc_subgrad = accs_iter(1,:,i);
    acc_ADMM = accs_iter(2,:,i);
    acc_ADMM_sc = accs_iter(3,:,i);
    acc_central = ones([1,length(beta_vec)])*acc_cent(i);
    ...
    semilogx(beta_vec,accs_iter(1,:,i),...
        'Color',[0.9290 0.6940 0.1250],'LineWidth',0.01);
    hold on
    errorbar(beta_vec,ones([1,length(beta_vec)])*acc_cent(i),...
        err_central(2,:),err_central(1,:),'k--','LineWidth',0.8)
    errorbar(beta_vec,accs_iter(1,:,i),err_subgrad(2,:),err_subgrad(1,:),...
        'Color',[0.9290 0.6940 0.1250],'LineWidth',0.8);
    errorbar(beta_vec,accs_iter(2,:,i),err_ADMM(2,:),err_ADMM(1,:),...
        'Color',[0.4940 0.1840 0.5560],'LineWidth',0.8);
    errorbar(beta_vec,accs_iter(3,:,i),err_ADMM_sc(2,:),err_ADMM_sc(1,:),...
    ...
    xlabel('$\beta_g$ Hyperparameter','Interpreter','latex')
    ...
    err_temp_central = ones([1,length(k_vec)])*std(acc_central_bds(i,:));

    acc_subgrad = accs_param(1,:,i);
    acc_ADMM = accs_param(2,:,i);
    acc_ADMM_sc = accs_param(3,:,i);
    acc_central = ones([1,length(k_vec)])*acc_cent(i);
    ...
    plot(k_vec,accs_param(1,:,i),'k--','LineWidth',0.01);
    hold on
    errorbar(k_vec,ones([1,length(k_vec)])*acc_cent(i),...
        err_central(2,:),err_central(1,:),'k--','LineWidth',0.8)
    errorbar(k_vec,accs_param(1,:,i),err_subgrad(2,:),err_subgrad(1,:),...
        'Color',[0.9290 0.6940 0.1250],'LineWidth',0.8);
    errorbar(k_vec,accs_param(2,:,i),err_ADMM(2,:),err_ADMM(1,:),...
        'Color',[0.4940 0.1840 0.5560],'LineWidth',0.8);
    errorbar(k_vec,accs_param(3,:,i),err_ADMM_sc(2,:),err_ADMM_sc(1,:),...
        'Color',[0.3010 0.7450 0.9330],'LineWidth',0.8);
    grid on
    xlabel('$\kappa_g$ Hyperparameter','Interpreter','latex')
    ylabel('mCCR','Interpreter','latex')
    title(titles(i),'Interpreter','latex')
    xticks(k_vec)
    axis([0,1,0.5,1])
