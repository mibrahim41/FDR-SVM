...
def save_exp_file(batch,P,G,N_train,N_test,weights_train,weights_test,class_sep,perc_g_vec,num_exp,max_iter_vec,
    ...
    filename = 'FDR-SVM_sensitivity_gro_'+str(int(timeit.default_timer()))+batch+'.mat'
...
acc_tensor_subgrad = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
acc_tensor_ADMM = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
acc_tensor_ADMM_sc = np.zeros([len(beta_vec),len(kappa_vec),num_exp])

runtime_tensor_subgrad = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
runtime_tensor_ADMM = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
runtime_tensor_ADMM_sc = np.zeros([len(beta_vec),len(kappa_vec),num_exp])

adj_tensor_subgrad = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
adj_tensor_ADMM = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
adj_tensor_ADMM_sc = np.zeros([len(beta_vec),len(kappa_vec),num_exp])
...
for i in range(num_exp):
    ...
    central_set, client_sets, test_set, central_set_norm, client_sets_norm, test_set_norm, Ng_vec = gen_data(imb_type='1', 
                                                                                                             perc_wrong_y=0)
    ...
    for m in range(len(beta_vec)):
        epsilon_vec_algs = np.zeros(G)
        for j in range(len(Ng_vec)):
            epsilon_vec_algs[j] = 1/(beta_vec[m]*Ng_vec[j])

        for k in range(len(kappa_vec)):
            params_subgrad = {'epsilon':epsilon_vec_algs, 'kappa':np.ones(G)*kappa_vec[k], 'pnorm':float('Inf'), 
                              'max_iter':180, 'stepsize':1e2}
            init_vars_subgrad = {'w':np.zeros(P)}


            params_ADMM = {'epsilon':epsilon_vec_algs, 'kappa':np.ones(G)*kappa_vec[k], 'pnorm':1,
                           'rho':1e-3, 'max_iter':100, 'tau':0}
            params_ADMM_sc = {'epsilon':epsilon_vec_algs, 'kappa':np.ones(G)*kappa_vec[k], 'pnorm':1,
                           'rho':1e-3, 'max_iter':100, 'tau':18*(1e-3)}
            ...
            acc_tensor_subgrad[m,k,i] = acc_subgrad
            runtime_tensor_subgrad[m,k,i] = stop_subgrad - start_subgrad
            adj_tensor_subgrad[m,k,i] = adj_subgrad
            ...
            acc_tensor_ADMM[m,k,i] = acc_ADMM
            runtime_tensor_ADMM[m,k,i] = stop_ADMM - start_ADMM
            adj_tensor_ADMM[m,k,i] = adj_ADMM
            ...
            acc_tensor_ADMM_sc[m,k,i] = acc_ADMM_sc
            runtime_tensor_ADMM_sc[m,k,i] = stop_ADMM_sc - start_ADMM_sc
            adj_tensor_ADMM_sc[m,k,i] = adj_ADMM_sc
